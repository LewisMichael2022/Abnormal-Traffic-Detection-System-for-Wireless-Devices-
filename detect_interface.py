...
inactive_timeout_setting=1 # 每筆flow最長幾秒就輸出
...
if __name__ == "__main__":
    ...
    if detect_mode:
        import joblib
        k_best = joblib.load('D:\\test_program\\final_project\\final_6\\kbest.save')
        model=load_model('D:\\test_program\\final_project\\final_6\\cnn_lstm_model.h5')
        scaler = joblib.load('D:\\test_program\\final_project\\final_6\\scaler.save')
        le = joblib.load('D:\\test_program\\final_project\\final_6\\label_encoder.save')
        window_size = 10
    ...
    streamer = NFStreamer(
        ...
        decode_tunnels=True,
        idle_timeout=idle_timeout_setting,      # 閒置多少秒就輸出 flow
        active_timeout=inactive_timeout_setting    # 最長多少秒就輸出 flow
    )
